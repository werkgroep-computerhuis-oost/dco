            ...
            try {
                ...
                open.addActionListener(e -> {
                        if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
                            try {
                                Desktop.getDesktop().browse(new URI("https://%s:%s/balie".formatted(serverIp, serverPort)));
                ...
                open.addActionListener(e -> {
                        if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
                            try {
                                Desktop.getDesktop().browse(new URI("https://%s:%s/educatie".formatted(serverIp, serverPort)));
                            } catch (Exception browserE) {
                                log.error("Exception: {}", browserE.getMessage());
                            }
                        }
                ...
                open.addActionListener(e -> {
                        if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
                            try {
                                Desktop.getDesktop().browse(new URI("https://%s:%s/werkplaats".formatted(serverIp, serverPort)));
                            } catch (Exception browserE) {
                                log.error("Exception: {}", browserE.getMessage());
                            }
                        }
                ...
                open.addActionListener(e -> {
                        if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
                            try {
                                Desktop.getDesktop().browse(new URI("https://%s:%s/admin".formatted(serverIp, serverPort)));
                            } catch (Exception browserE) {
                                log.error("Exception: {}", browserE.getMessage());
                            }
                        }
